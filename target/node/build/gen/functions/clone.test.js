...
describe_1.describe("clone", async ({ assert, inspect }) => {
...
describe_1.describe("clone, shallow", async ({ assert, inspect }) => {
    const original = { foo: "foo", bar: "bar" };
    const copy = clone_1.clone(original);
    copy.bar = "BAZ";
    assert({
...
describe_1.describe("clone, deep", async ({ assert, inspect }) => {
...
describe_1.describe("clone, undefined", async ({ assert }) => {
...
describe_1.describe("clone, null", async ({ assert }) => {
    const value = null;
    const original = {
        foo: "foo",
        bar: null,
...
describe_1.describe("clone, date", async ({ assert, inspect }) => {
...
describe_1.describe("clone, array", async ({ assert, inspect }) => {
...
describe_1.describe("clone, recursive", async ({ assert, inspect }) => {
    const a = {};
    a.a = a;
    const original = [a];

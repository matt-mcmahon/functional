...
import { curryN, signatures, implementation } from "./curry-n.ts";
...
describe(
  ...
  async ({ assert, inspect }) => {
    ...
    {
      const given = inspect`join0.arity`;
      const actual = join0.arity;
      const expected = 1;
      assert({ given, actual, expected });
    }
    ...
    {
      const given = inspect`join1.arity`;
      const actual = join1.arity;
      const expected = 1;
      assert({ given, actual, expected });
    }

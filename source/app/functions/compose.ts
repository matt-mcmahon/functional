 ...
 *
 * ```
 * fluent(h).then(g).then(f).call(a) <=> h(g(f(a)))
 * ```
 *
 ...
 *
 * ```
 * fluent(h).then(g).then(f)(a)
 * ```
 * and if you assign a fluent chain to a variable you may treat it as a normal
 * function. For example:
 * ```
 * const hgf = fluent(h).then(g).then(f)
 * hgf(a) <=> hgf.call(a)
 * ```
 */
const compose = Object.assign( // deno-lint-ignore no-explicit-any
